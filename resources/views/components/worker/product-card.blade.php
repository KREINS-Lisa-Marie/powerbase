...
<article class="product-card text-white border-r-16">
    <div class="d-flex flex-col-reverse">
        ...
        @if($product_image)
            <img src="{{Storage::disk('s3')->url('images/products/variants/288x288/'.basename($product_image))}}" {{--{!! asset('storage/images/products/variants/288x288/'.basename($product_image)) !!}"--}} alt="{{__('admin/products.the_product_image')}}"
